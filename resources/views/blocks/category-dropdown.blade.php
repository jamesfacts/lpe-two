@if($block->preview)
    <h3 style="font-family: Rubik, sans-serif; margin-top: 2rem;">Category Dropdown</h3>
    <select name="" id="">
        <option value="">Blog categories will appear here...</option>
    </select>
@else
<div class="mt-3">
@if($topics)
<span class="uppercase block mb-1">Sort By Category</span>
<div class="flex {{ $block->classes }} relative inline-block text-left">
    ...
    >
        ...
        <div
            ...
            style="display: none;"
            class="absolute left-0 mt-2 w-full bg-white shadow-md z-10"
        >

        @foreach($topics as $taxItem)
            <a href="{!! $taxItem->url !!}" 
               class="uppercase w-full block first-of-type:pt-2 text-tahini-700 underline
